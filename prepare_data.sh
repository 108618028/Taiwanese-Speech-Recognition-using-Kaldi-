...
#utils/subset_data_dir_tr_cv.sh --cv-spk-list data/train/cv.spk data/all data/train data/test
#utils/subset_data_dir_tr_cv.sh --cv-spk-list data/train/ data/all data/train data/test
...
#find -L $eval_key_dir -name *.txt -exec sh -c 'x={}; y=$(basename -s .txt $x); printf "%s " $y; cat $x'    \; | sed 's/\xe3\x80\x80\|\xc2\xa0//g' | dos2unix > data/test/text
